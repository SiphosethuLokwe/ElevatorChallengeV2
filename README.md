...
To update the database use this command from the Web project folder (replace `ElevatorChallenge` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../ElevatorChallenge.Infrastructure/ElevatorChallenge.Infrastructure.csproj -s ElevatorChallenge.Web.csproj
```
